    ...
    if(isset($_POST['action']))
{
    ...
    if(in_array($fileType , $validTypes) && $userpassword == $userrepassword )
    {
        ...
        $q= "insert into qi_students(qi_username , qi_mail , qi_password , qi_pp , qi_qid) values ('$username' , '$usermail' , '$userpassword' , '$imgPath' ,'17')";
      mysqli_query($con , $q);
        
        
        $q1 = "select qi_id from qi_students order by qi_id  desc limit 1";
$test = mysqli_query($con , $q1);  
$q_id = mysqli_fetch_array($test);
 $q_id =  $q_id['qi_id'];
$_SESSION['studentId']= $q_id;

        
        $_SESSION['isLogin']=true;  

                    ?><script><?php echo("location.href ='studentprofile.php';");?></script><?php 
